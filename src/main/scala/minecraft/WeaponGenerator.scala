...
object MinecraftToolsGenerator {
  ...
  private def coreRecipe(
      ingredient: Ingredient,
      t: ToolType,
      requiredQuantity: Int): (Weapon, Option[Ingredient]) = {
    ...
    remainingQuantity match {
      ...
      case _ =>
        (Weapon(t, ingredient.material),
         Some(Ingredient(remainingQuantity, ingredient.material)))
    }
  }

  def swordRecipe: Recipe =
    Ingredient => coreRecipe(Ingredient, ToolType.SWORD, 2)

  def axeRecipe: Recipe = Ingredient => coreRecipe(Ingredient, ToolType.AXE, 3)

  def shovelRecipe: Recipe =
    Ingredient => coreRecipe(Ingredient, ToolType.SHOVEL, 1)

  def craft(recipe: Recipe,
            ingredient: Ingredient): Either[String, WeaponAndIngredients] =
    recipe(ingredient)._2 match {
      case Some(_) => Right(recipe(ingredient))
      case None    => Left("Dude, can't build this tool")
    }

  def uncraft: Weapon => Ingredient = { Weapon =>
